...
namespace SpectrumV1.Views.Common.Countries
{
	public partial class CityEditForm : XtraForm
	{
		...
		private IList<DistrictModel> _districtModel = new List<DistrictModel>();
		...
		private async void btnSave_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				if (string.IsNullOrEmpty(_cityModel._id))
				{
					...
					var newId = await _cityRepository.AddNewCityAsync(_cityModel);
				}
				else
				{
					_logInfoRepository.UpdateLogInfo(_cityModel);

					await _cityRepository.UpdateCityAsync(_cityModel);
				}
