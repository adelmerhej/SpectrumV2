	...
	public partial class ProvinceEditForm : XtraForm
	{
		...
		private async void btnSave_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				_provinceModel = (ProvinceModel)bsProvince.Current;


				if (string.IsNullOrEmpty(_provinceModel._id))
				{
					_logInfoRepository.CreateLogInfo(_provinceModel);

					var newId = await _provinceRepository.AddNewProvinceAsync(_provinceModel);
				}
				else
				{
					_logInfoRepository.UpdateLogInfo(_provinceModel);

					await _provinceRepository.UpdateProvinceAsync(_provinceModel);
				}
